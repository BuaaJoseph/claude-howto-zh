...
Claude Code CLI（命令行界面）是与 Claude Code 交互的主要方式。它提供了强大的选项来运行查询、管理会话、配置模型，以及将 Claude 集成到你的开发工作流中。
...
| `claude "query"` | 启动带初始提示的 REPL | `claude "解释这个项目"` |
...
| `--bare` | 极简模式（跳过 hooks、skills、插件、MCP、自动记忆、CLAUDE.md） | `claude --bare` |
...
| `--fallback-model` | 过载时的自动模型回退 | `claude -p --fallback-model sonnet "查询"` |
| `--agent` | 指定会话的代理 | `claude --agent my-custom-agent` |
...
```bash
# 完全自定义角色
claude --system-prompt "你是一个高级安全工程师。专注于漏洞。"

# 追加特定指令
claude --append-system-prompt "代码示例中始终包含单元测试"
...
| `--max-budget-usd` | 打印模式最大花费 | | `claude -p --max-budget-usd 5.00 "查询"` |
...
```bash
# 加载 GitHub MCP 服务器
claude --mcp-config ./github-mcp.json "列出打开的 PR"

# 严格模式 - 仅指定的服务器
claude --strict-mcp-config --mcp-config ./production-mcp.json "部署到 staging"
```
...
# 带自定义消息的分支
claude -r "feature-auth" --fork-session "用不同架构测试"
```
...
`--agents` 标志接受一个 JSON 对象来定义会话的自定义子代理。
...
{
  "code-reviewer": {
    "description": "专家代码审查者。在代码更改后主动使用。",
    "prompt": "你是一个高级代码审查者。专注于代码质量、安全和最佳实践。",
    "tools": ["Read", "Grep", "Glob", "Bash"],
    "model": "sonnet"
  },
  "debugger": {
    "description": "错误和测试失败的调试专家。",
    "prompt": "你是一个调试专家。分析错误、识别根本原因并提供修复。",
    "tools": ["Read", "Edit", "Bash", "Grep"],
    "model": "opus"
  },
  "documenter": {
    "description": "生成指南的文档专家。",
    "prompt": "你是一个技术作家。创建清晰、全面的文档。",
    "tools": ["Read", "Write"],
    "model": "haiku"
  }
}
...
当存在多个代理定义时，它们按此优先级顺序加载：
1. **CLI 定义的**（`--agents` 标志）- 会话特定
2. **用户级别**（`~/.claude/agents/`）- 所有项目
3. **项目级别**（`.claude/agents/`）- 当前项目

CLI 定义的代理会为会话覆盖用户和项目代理。
...
在 CI/CD 管道中使用 Claude Code 进行自动代码审查、测试和文档生成。
...
```bash
# 审查特定文件
cat src/auth.ts | claude -p "审查这个认证代码的安全问题"

# 生成文档
cat src/api/*.ts | claude -p "以 markdown 格式生成 API 文档"

# 查找 TODO 并排序优先级
grep -r "TODO" src/ | claude -p "按重要性排序这些 TODO"
```
...
```bash
# 启动功能分支会话
claude -r "feature-auth" "来实现用户认证"

# 之后继续会话
claude -r "feature-auth" "��加密码重置功能"

# 分支尝试替代方案
claude --resume feature-auth --fork-session "尝试 OAuth"
...
{
  "reviewer": {
    "description": "PR 审查的代码审查者",
    "prompt": "审查代码质量、安全性和可维护性。",
    "model": "opus"
  },
  "documenter": {
    "description": "文档专家",
...
# 提取多个字段
claude -p --output-format json "描述这个项目" | jq -r '.{name, version, description}'
...
# 处理整个数组
claude -p --output-format json "查找 todos" | jq '.todos | length'
...
| `CLAUDE_STREAM_IDLE_TIMEOUT_MS` | 流空闲超时毫秒数 |
...
| 极简模式 | `claude --bare "快速查询"` |
| 有预算的运行 | `claude -p --max-budget-usd 2.00 "分析代码"` |
...
- 使用 `--json-schema` 强制结构
- 在提示中添加明确的 JSON 指令
- 使用 `--output-format json`（而不仅仅是提示中要求 JSON）
...
- 谨慎使用 `--dangerously-skip-permissions` 进行自动化
